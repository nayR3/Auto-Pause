...
$execute {
    listenForSettingChanges<std::filesystem::path>("media-app", [](std::filesystem::path mediaPath) {
        if (isAppRunning(mediaPath)) {
            log::info("App detected!");
            pid = getProcessPID(mediaPath);
            hwnd = findWindowByPID(pid);
...
void sendKey() {
    if (!pid || !hwnd) {
        ...
        return;
    } // check if media app is running

    // get value of pause-key setting, exit if not set
    auto binds = Mod::get()->getSettingValue<std::vector<geode::Keybind>>("pause-key");
    if (binds.empty()) return;

    // convert value into wparam
    geode::Keybind first = binds[0];
    auto key = static_cast<WPARAM>(first.key);
